...
def verify_token(token: str) -> str:
    """Verify Firebase ID token and return user ID, or raise HTTP 401."""
    try:
...
def github_api(query: str, variable=None):
    """Execute GitHub GraphQL API query with timeout."""
    ...
    response = requests.post(url=url, json=json_data, headers=header, timeout=20)
    if response.status_code == 200:
        return response.json()
    else:
        raise Exception(f"GitHub API query failed {response.status_code}: {response.text}")


def jupternotebook_cleaner(text: str) -> str:
    if not text:
        return ""
    ...
    try:
        ...
        logger.debug("Failed to parse Jupyter Notebook JSON: %s", e)
        return ""
...
def sanitize_data(data):
    ...
    - tuple -> list
    - numpy numeric types -> int / float
    """
    if isinstance(data, dict):
        return {k: sanitize_data(v) for k, v in data.items()}
    elif isinstance(data, (set, frozenset)):
        return sorted([sanitize_data(v) for v in data])
